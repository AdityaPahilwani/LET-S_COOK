...
import {
  ...
  TouchableWithoutFeedback, Keyboard, TouchableOpacity, LayoutAnimation, UIManager, ScrollView
} from 'react-native';
  ...
  const Signupview = () => {
    ...
    return (
      <View style={styles.signup}>
        ...
        <Text style={styles.signUpText}>Switch to
            <TouchableWithoutFeedback onPress={() => {
            LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut);
            setIsSignup(!isSignup)
          }}>
    ...
    if (isSignup) {
      ...
      if (formState.formIsValid === true) {
        if (formState.input.pass === formState.input.conpass) {
          ...
          action = authActions.signup(formState.input.email, formState.input.pass);
          console.log('innn');
        }
        else {
      ...
      if (formState.validities.email === true && formState.validities.pass === true) {
              ...
              <ActivityIndicator size="small" color={Color.primary} />
                ...
                <>
                  < TouchableOpacity style={styles.button} onPress={authHandler}>
                    <Text style={{ color: Color.dark, }}>{isSignup ? 'Sign up' : 'Sign in'} </Text>
                  </TouchableOpacity>

                  <Text style={styles.text}>or Sign in with facebook or google</Text>
                  <View style={styles.actions}>
                    <TouchableOpacity style={styles.roundButton} onPress={() => { console.log(formState.input.email + '   ' + formState.input.pass + ' jj') }}>
                      <FontAwesome name="facebook" size={wp('7%')} color={Color.primary} />
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.roundButton} onPress={googlehandler}>
                      <AntDesign name="google" size={wp('7%')} color={Color.primary} />
                    </TouchableOpacity>
                  </View>
                </>
              )}
